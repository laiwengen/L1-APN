...
#if 1 // put stuff
static inline void iapputc(uint8_t c)
{
	while((IAP_UART->ISR&UART_FLAG_TXE) == 0);
	IAP_UART->TDR = c;
}

static void iapputs(uint8_t* str)
{
	while(*str)
	{
		iapputc(*str);
	...
	switch (ih->type)
	{
		...
		case 0x10:
			...
			if ( ih->length > 0 && ih->length<=2)
			{
				...
				iapputs("IAP Erease pages, address is: 0x");
				puthex(ih->address,16);
				iapputs(" length: 0x");
				puthex(length,16);
				iapputs(".\r\n");
		...
		case 0x11:
			...
			if ( ih->length > 0 && ih->length<=2)
			{
				ih->type = 0x00;
				uint16_t length = msbBytes(ih->data,ih->length);
				iapputs("IAP Read flash, address is: 0x");
				puthex(ih->address,16);
				iapputs(" length: 0x");
				puthex(length,16);
				iapputs(":\r\n");
